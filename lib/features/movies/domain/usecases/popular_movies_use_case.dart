...
class PopularMoviesUseCase extends UseCaseBase<Movies, int> {
  PopularMoviesUseCase(this._moviesRepository);
  ...
  Future<Either<MovieAppFailure, Success<Movies>>> call(params) async {
    try {
      return success(await _moviesRepository.getPopularMovies(params));
    } catch (error) {
