...
class NowPlayingMoviesUseCase extends UseCaseBase<Movies, int> {
  NowPlayingMoviesUseCase(this._moviesRepository);
  ...
  Future<Either<MovieAppFailure, Success<Movies>>> call(params) async {
    try {
      return success(await _moviesRepository.getNowPlayingMovies(params));
    } catch (error) {
